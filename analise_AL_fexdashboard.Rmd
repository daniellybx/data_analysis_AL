...
dados_AML$tx_inc = round(as.double(dados_AML$Cumulative_cases)/as.double(dados_AML$Pop)*100000, 2)
dados_AML$tx_obt = round(as.double(dados_AML$Cumulative_deaths)/as.double(dados_AML$Pop)*100000, 2)
```
...
obitos+ 
  geom_bar(stat="identity", fill = '#f70a0a')+
  scale_x_date(breaks="5 days", labels=date_format("%d/%m"))+
  ggtitle('Número de óbitos confirmados de Covid-19 por dia, América Latina, 24 de maio de 2020')+
  ylab("Casos") + xlab("Dia") +
  theme_bw() 
```
...
Row 
-------------------------------------
### Taxa de incidência na América Latina 

```{r, TX_INC}
#gráfico de linhas da taxa de incidência
tx_inc = ggplot(dados_AML, aes(day, tx_inc))
tx_inc+
  geom_line(color = '#3434eb', size = 1.5)+
  scale_x_date(breaks="5 days", labels=date_format("%d/%m"))+
  ggtitle('Taxa de incidência acumulada por Covid-19, América Latina, 24 de maio de 2020')+
  ylab("Taxa de incidência") + xlab("Dia") +
  theme_bw()
```
